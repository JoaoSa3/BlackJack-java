...
public class BlackjackGame {
    ...
    public void startRound() {
        ...
        dealerHoleHidden = true; outcome = ""; state = State.PLAYER_TURN;
        player.add(deck.draw()); dealer.add(deck.draw());
        player.add(deck.draw()); dealer.add(deck.draw());
        if (player.isBlackjack() || dealer.isBlackjack()) stand();
    }
    public void hit() {
        if (state != State.PLAYER_TURN) return;
        player.add(deck.draw());
        if (player.isBust()) { dealerHoleHidden = false; state = State.ROUND_OVER; outcome = "Dealer wins"; }
    }
    public void stand() {
        if (state == State.ROUND_OVER) return;
        dealerHoleHidden = false; state = State.DEALER_TURN;
